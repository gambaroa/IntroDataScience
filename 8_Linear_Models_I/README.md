...
-   [Data `gapminder` from `dslabs`: this data set reports health and
    income outcomes of 184 countries from 1960
    to 2016.](#data-gapminder-from-dslabs-this-data-set-reports-health-and-income-outcomes-of-184-countries-from-1960-to-2016)
    ...
    -   [Linear Model](#linear-model)
        ...
        -   [fitting model checking
            assumptions](#fitting-model-checking-assumptions)
...
# Data `gapminder` from `dslabs`: this data set reports health and income outcomes of 184 countries from 1960 to 2016.
...
### fitting model checking assumptions
...
![](8th-data-practical_files/figure-gfm/model-1.png)<!-- --> From the
Plot we can see that there is no linear model since the line curve a
little. so I decided to perform a Logarithmic transformation which is a
convenient means of transforming a highly skewed variable into a more
normalized dataset. When modeling variables with non-linear
relationships, the chances of producing errors may also be skewed
negatively.
...
![](8th-data-practical_files/figure-gfm/logmodel-1.png)<!-- --> As
expected now the relation seem to be more linear compared to the
original variables. ù
...
![](8th-data-practical_files/figure-gfm/linearityassump-1.png)<!-- -->

Seem from this plot that there is no linear relation between the
varibles ### Homogeneity of variance

``` r
plot(logmodel, 3)
```

![](8th-data-practical_files/figure-gfm/assumption-1.png)<!-- -->
...
![](8th-data-practical_files/figure-gfm/assumptionnorm-1.png)<!-- -->
...
loginfant<-log(datait$infant_mortality)
loglife<-log(datait$life_expectancy)
logmodel2 <-glm(loginfant ~ loglife, datait, family="gaussian")
summary(logmodel2)
```

    ## 
    ## Call:
    ## glm(formula = loginfant ~ loglife, family = "gaussian", data = datait)
    ## 
    ## Deviance Residuals: 
    ##      Min        1Q    Median        3Q       Max  
    ## -0.12984  -0.03806  -0.01219   0.04029   0.11885  
    ...
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## (Dispersion parameter for gaussian family taken to be 0.003808233)
    ## 
    ##     Null deviance: 42.77722  on 55  degrees of freedom
    ## Residual deviance:  0.20564  on 54  degrees of freedom
    ##   (1 observation deleted due to missingness)
    ## AIC: -149.07
    ## 
...
ggplot(datait, aes(loginfant, loglife)) +
  geom_point(size=2, aplha= 1) +
 geom_smooth(method = "gaussian", se= TRUE) +
 theme_classic()
```

![](8th-data-practical_files/figure-gfm/glm-1.png)<!-- -->
